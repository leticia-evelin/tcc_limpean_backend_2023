...
var require_json = __commonJS({
  "node_modules/body-parser/lib/types/json.js"(exports, module2) {
    ...
    function json(options) {
      ...
      return function jsonParser2(req, res, next) {
        if (req._body) {
...
function validateTypesJson2(json) {
  try {
    const diarista = json;
    console.log(json);
...
function validatePhoneWithDDD2(ddd, phone) {
  let numberPhone = `${ddd} ${phone}`;
  let telefone = numberPhone.replace(/\D/g, "");
  if (!(telefone.length >= 10 && telefone.length <= 11))
    return false;
...
function validateDateBirth2(dataNascimento) {
  let status = true;
  const dataNascRegex = /^\d{4}-\d{2}-\d{2}$/;
  if (!dataNascimento.match(dataNascRegex)) {
    return false;
...
function validadeAddress2(address) {
  let statusAddress = true;
  if (typeof address.state !== "number" || address.state <= 0 || !address.city || address.city === "" || !address.cep || address.cep === "" || !address.district || address.district === "" || typeof address.houseNumber !== "string" || typeof address.publicPlace !== "string" || address.publicPlace === "" || address.complement !== null && typeof address.complement !== "string") {
    statusAddress = false;
  }
...
var { PrismaClient: PrismaClient2 } = require("@prisma/client");
var prisma2 = new PrismaClient2();
var registerUser2 = async function(dataBody) {
  try {
    const verifyDiarist = await prisma2.tbl_diarista.findFirst({
      where: {
        OR: [
          { email: dataBody.email.toLowerCase() },
          { cpf: dataBody.cpf }
    ...
    if (!verifyDiarist) {
      const tbl_cidade = await prisma2.tbl_cidade.create({
        data: {
          nome: dataBody.address.city,
          id_estado: dataBody.address.state
        }
      });
      const tbl_endereco = await prisma2.tbl_endereco.create({
        data: {
          logradouro: dataBody.address.publicPlace,
          bairro: dataBody.address.district,
          cep: dataBody.address.cep,
      ...
      const tbl_diarista = await prisma2.tbl_diarista.create({
        data: {
          nome: dataBody.nameUser,
          cpf: dataBody.cpf,
          data_nascimento: new Date(dataBody.birthDate),
...
app.use(server);
app.listen(port, () => console.log("Servidor Aguardando requisi\xE7\xF5es na porta 8080"));
